...
def fitxspec(obsmode,a,b,Etem,t,dirstr):
    ...
    if obsmode == 'Nor':
        obs = 0
        AllData.ignore("1: **-100.0,600.0-**")
    elif obsmode == 'GRB':
        obs = 1
        AllData.ignore("1: **-400.0,2800.0-**")
    ...
    lstr1 = str(obsmode)+' timescal:'+str(t)+' a:'+str(a)+' b:'+str(b)+' Epeak:'+str(Etem*(a+2))+' flulim:'+str(flu)+'*10^-7 erg/cm^2'
    lstr1 += '\n'
...
def run_task_xspec(ra,dec,trigtime,obsmode,t,t_bkg_l_b,t_bkg_l_f,t_bkg_r_b,t_bkg_r_f,dirstr):
    ...
    for i in range(0,3):
        ...
        b = betas[i]
        Etem = Etems[i]
        fitxspec(obsmode,a,b,Etem,t,dirstr)
