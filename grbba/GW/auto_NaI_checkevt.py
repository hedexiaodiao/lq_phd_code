...
import os
import sys
sys.path.append('/sharefs/hbkg/user/luoqi/GRB/soft/make_pha')
import make_pha_v1
import time
import auto_NaI_calclim
from shutil import copyfile


def run_task(ra,dec,trigtime,obsmode,t,t_bkg_l_b,t_bkg_l_f,t_bkg_r_b,t_bkg_r_f,dirstr,detector):
    print('run_task')
    ...
    try:
        if int(t)==1:
            ...
            t_src_b = 0
            t_src_f = 10
        os.chdir('/sharefs/hbkg/user/luoqi/GRB/grbba/GW/'+str(trigtime.datetime.year)+'{:02d}'.format(trigtime.datetime.month)+'{:02d}'.format(trigtime.datetime.day)+str(dirstr)+'/t'+str(int(t)))
        make_pha_v1.make_pha(str(trigtime.value), t_bkg_l_b, t_bkg_l_f, t_src_b, t_src_f, \
             t_bkg_r_b, t_bkg_r_f, None, detector, 'D')
        ...
        if detector=='NaI':
            copyfile('/sharefs/hbkg/user/luoqi/GRB/soft/make_pha/DA.rsp','./DA.rsp')
        else:
            os.chdir('/sharefs/hbkg/user/luoqi/GRB/grbba/GW/'+str(trigtime.datetime.year)+'{:02d}'.format(trigtime.datetime.month)+'{:02d}'.format(trigtime.datetime.day)+str(dirstr))
            if obsmode == 'GRB':
            ...
            if_rsp_exist=0
            while if_rsp_exist==0:
                time.sleep(15)
                if os.path.exists('/sharefs/hbkg/user/luoqi/GRB/grbba/GW/'+str(trigtime.datetime.year)+'{:02d}'.format(trigtime.datetime.month)+'{:02d}'.format(trigtime.datetime.day)+str(dirstr)+'/HXMTrspg_v9_FITS/DA.rsp')==1:
                    copyfile('/sharefs/hbkg/user/luoqi/GRB/grbba/GW/'+str(trigtime.datetime.year)+'{:02d}'.format(trigtime.datetime.month)+'{:02d}'.format(trigtime.datetime.day)+str(dirstr)+'/HXMTrspg_v9_FITS/DA.rsp','./t'+str(int(t))+'/FITS/DA.rsp')
                    if_rsp_exist = 1
            os.chdir('/sharefs/hbkg/user/luoqi/GRB/grbba/GW/'+str(trigtime.datetime.year)+'{:02d}'.format(trigtime.datetime.month)+'{:02d}'.format(trigtime.datetime.day)+str(dirstr)+'/t'+str(int(t))+'/FITS')
            
            
        
        #subprocess.Popen('cp /sharefs/hbkg/user/luoqi/GRB/soft/make_pha/DA.rsp ./FITS/DA.rsp', shell=True)
        print('ok, then start to calclim')
        auto_NaI_calclim.timerun_xspec(ra,dec,trigtime,obsmode,t,t_bkg_l_b,t_bkg_l_f,t_bkg_r_b,t_bkg_r_f,dirstr)
        '''
        #write run_spec.pro
...
def check_file(ra,dec,trigtime,obsmode,t,t_bkg_l_b,t_bkg_l_f,t_bkg_r_b,t_bkg_r_f,dirstr,detector):
            ...
            if os.path.exists('HXMT_'+str(trigtime.datetime.year)+'{:02d}'.format(trigtime.datetime.month)+'{:02d}'.format(trigtime.datetime.day)+'T'+\
                ...
                while ver_att<6:
                    if os.path.exists('HXMT_'+str(trigtime.datetime.year)+'{:02d}'.format(trigtime.datetime.month)+'{:02d}'.format(trigtime.datetime.day)+'T'+\
                        ...
                        flag = run_task(ra,dec,trigtime,obsmode,t,t_bkg_l_b,t_bkg_l_f,t_bkg_r_b,t_bkg_r_f,dirstr,detector)
                        break
                    else:
                        ver_att += 1
                        flag = 1
...
def timerun(ra,dec,trigtime,obsmode,t,t_bkg_l_b,t_bkg_l_f,t_bkg_r_b,t_bkg_r_f,dirstr,detector):
    ...
    while flag==1:
        ...
        print(now,j)        
        flag = check_file(ra,dec,trigtime,obsmode,t,t_bkg_l_b,t_bkg_l_f,t_bkg_r_b,t_bkg_r_f,dirstr,detector)
        j = j+1
        time.sleep(30)
...
if __name__ == "__main__":
    ...
    if os.path.exists('/sharefs/hbkg/user/luoqi/GRB/grbba/GW/'+str(trigtime.datetime.year)+'{:02d}'.format(trigtime.datetime.month)+'{:02d}'.format(trigtime.datetime.day)+str(dirstr))==1:
        print('please change the argument \'--dirstr\'!!!\nor if spectum files exists in this dir already,\nyou can use command:\n'+\
              'python /sharefs/hbkg/user/luoqi/GRB/grbba/GW/auto_NaI_calclim.py --ra-deg '+str(ra)+' --dec-deg '+str(dec)+' --trigtime '+str(trigtime.value)+\
    ...
    #else:
    timerun(ra,dec,trigtime,obsmode,t,t_bkg_l_b,t_bkg_l_f,t_bkg_r_b,t_bkg_r_f,dirstr,detector)
